...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 30-01-2021 a las 08:28:22
...
--
-- Estructura Stand-in para la vista `reportlecturaspdf`
-- (Véase abajo para la vista actual)
--
CREATE TABLE `reportlecturaspdf` (
`num_cuenta` varchar(20)
,`nombre` varchar(50)
,`apellido` varchar(30)
);
...
(1, 'Admin Users', 'Admin', 'd033e22ae348aeb5660fc2140aec35850c4da997', 1, 'hhskvuf1.jpg', 1, '2021-01-29 08:36:58'),
...
--
-- Estructura para la vista `reportlecturaspdf`
--
DROP TABLE IF EXISTS `reportlecturaspdf`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `reportlecturaspdf`  AS  select `inv_cliente`.`num_cuenta` AS `num_cuenta`,`inv_cliente`.`nombre` AS `nombre`,`inv_cliente`.`apellido` AS `apellido` from (`inv_cliente` join `lecturas` on(`inv_cliente`.`num_cuenta` = `lecturas`.`num_cuenta`)) ;
